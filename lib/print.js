...
console.log(`┏━━◈ ${wm}  ◈───⪼ 
┃ BOT: ${chalk.blue('%s')}
┃ HORA: ${chalk.black(chalk.cyan('%s'))}ㅤ
┃ PESO: ${chalk.black(chalk.cyan('%s'))} 
┃ USUARIO: ${chalk.magenta('%s [%s %sB]')}
┃ CODIGO: ${chalk.white('%s')} ${chalk.yellow('%s%s')} 
┃ ESTADO: ${chalk.magenta('%s')} ${chalk.black(chalk.red('%s'))}
┗━━━━━━━━━━━━⪼`.trim(),
            
me + ' ~' + conn.user.name + `${conn.user.jid == global.conn.user.jid ? '' : ' (𝗠𝗬 - 𝗦𝗨𝗕 𝗕𝗢𝗧)'}`,
