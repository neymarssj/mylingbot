...
if (!text && !m.quoted) return m.reply(lenguajeMY.smsBCMensaje(usedPrefix, command))        
...
let locale = lenguajeMY.lenguaje()
let dia = d.toLocaleDateString(locale, { weekday: 'long' })
let fecha = d.toLocaleDateString(lenguajeMY.lenguaje(), { day: 'numeric', month: 'numeric', year: 'numeric' })
let mes = d.toLocaleDateString(lenguajeMY.lenguaje(), { month: 'long' })
let año = d.toLocaleDateString(lenguajeMY.lenguaje(), { year: 'numeric' })
...
await m.reply(lenguajeMY.smsChatGP1())   
for (let i = 0; i < groups.length; i++) {
const id = groups[i];
const infoGP = lenguajeMY.smsChatGP2(readMS, dia, mes, año, fecha, tiempo)
...
await m.reply(lenguajeMY.smsChatGP3(totalGP))
}     
