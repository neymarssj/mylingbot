...
if (!text) return await conn.reply(m.chat, `${lenguajeMY.smsMalused2()} ⊱ *${usedPrefix + command} Bellyache*`, fkontak, m)
try {
...
await conn.reply(m.chat, `${lenguajeMY['smsMalError3']()}#report ${usedPrefix + command}\n\n${wm}`, fkontak, m)
...
let frep = { contextInfo: { externalAdReply: {title: wm, body: author, sourceUrl: redesMenu.getRandom(), thumbnail: await(await fetch(gataMenu.getRandom())).buffer() }}}
if (!text) return await conn.reply(m.chat, `${lenguajeMY.smsMalused2()} ⊱ *${usedPrefix + command} Bellyache*`, fkontak, m)
 //conn.sendButton(m.chat, lenguajeMY.smsMalused2(), `⊱ *${usedPrefix + command} Bellyache*`, null, [[lenguajeMY.smsConMenu(), `${usedPrefix}menu`]], fkontak, m)
try {
...
//await conn.sendButton(m.chat, '🎧 *Ｓ Ｐ Ｏ Ｔ Ｉ Ｆ Ｙ* 🎧', spotifyi, thumbnail, [[lenguajeMY.smsConMenu(), `${usedPrefix}menu`]], m, frep)
